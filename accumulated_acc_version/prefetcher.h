...
class Prefetcher
{
   public:
      ...
      // For most prefetchers just do nothing. It is only used for prefetcher
      // that need prefetch information feedback.
      virtual void returnPrefetchIssuedInfo(IntPtr prefetch_pc, IntPtr current_address) {return;}
      virtual void returnPrefetchHitInfo(IntPtr prefetch_pc, IntPtr current_address) {return;}
      virtual void returnPhaseInfo(uint32_t current_phase) {return;}
      virtual void returnCacheMissInfo(IntPtr prefetch_pc, IntPtr current_address) {return;}
      virtual void returnMemoryUtilization(double spare) {return;}
