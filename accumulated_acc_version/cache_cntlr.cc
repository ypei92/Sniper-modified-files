...
CacheCntlr::doPrefetch(IntPtr prefetch_address, IntPtr prefetch_pc, SubsecondTime t_start)
{
   ...
   m_master->m_prefetcher->returnPrefetchIssuedInfo(temp_prefetch_pc, prefetch_address);
...
CacheCntlr::processShmemReqFromPrevCache(CacheCntlr* requester, Core::mem_op_t mem_op_type, IntPtr address, bool modeled, bool count, Prefetch::prefetch_type_t isPrefetch, SubsecondTime t_issue, bool have_write_lock, IntPtr ca_pc)
{
   ...
   if (count)
   {
    ...
    if (count && isPrefetch == Prefetch::NONE ) {
        ...
        if (total_demand_accesses%phase_length == 0) {
            ...
            clearOnePhase();
            current_phase++;
            m_master->m_prefetcher->returnPhaseInfo(current_phase);
        }
    }
   ...
   if (cache_hit)
   {
      if (isPrefetch == Prefetch::NONE && cache_block_info->hasOption(CacheBlockInfo::PREFETCH))
      {
         ...
         */
         ...
         m_master->m_prefetcher->returnPrefetchHitInfo(prefetch_pc, address);
   ...
   {
      if (count) {
         m_master->m_prefetcher->returnCacheMissInfo(ca_pc, address);
      }
    ...
    */
    ...
    /*
    if (count && !cache_hit ) {
        m_master->m_prefetcher->returnCacheMissInfo(address);
    }
    */
   ...
   {
        m_master->m_prefetcher->returnMemoryUtilization(bandwidth_usage);
