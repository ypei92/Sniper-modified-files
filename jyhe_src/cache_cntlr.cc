...
LOG_ASSERT_ERROR(offset + data_length <= getCacheBlockSize(), "access until %u > %u", offset + data_length, getCacheBlockSize());
   ...
   {
      if(stats.pc_accesses.find(ca_pc) == stats.pc_accesses.end())
	{
		stats.pc_hits[ca_pc] = 0;
		stats.pc_accesses[ca_pc] = 0;
      }
         if (! cache_hit) {
      	   stats.pc_accesses[ca_pc]++;
         }
   }*/
   }
...
MYLOG("access done");
   ...
   if (count && modeled && m_master->m_prefetcher && mem_op_type != Core::WRITE)
   {
      trainPrefetcher(ca_pc, ca_address, cache_hit, prefetch_hit, t_start);
   }
...
void
CacheCntlr::trainPrefetcher(IntPtr pc, IntPtr address, bool cache_hit, bool prefetch_hit, SubsecondTime t_issue)
{
   ScopedLock sl(getLock());

   // Always train the prefetcher
   std::vector<IntPtr> prefetchList = m_master->m_prefetcher->getNextAddress(pc, address, cache_hit, m_core_id);
...
CacheCntlr::Prefetch(SubsecondTime t_now)
{
   ...
   for(unsigned int i=0; i<prefetchList.size(); i++)
   {
       ...
       if (!operationPermissibleinCache(prefetchList[i], Core::READ))
           doPrefetch(prefetchList[i], prefetchPcList[i], issue_time[i]);
       else if(overlapping)
           redundant_in_progress++; 
       else
           redundant_in_cache++; 
...
void
CacheCntlr::doPrefetch(IntPtr prefetch_address, IntPtr prefetch_pc, SubsecondTime t_start)
{
...
CacheCntlr::processShmemReqFromPrevCache(CacheCntlr* requester, Core::mem_op_t mem_op_type, IntPtr address, bool modeled, bool count, Prefetch::prefetch_type_t isPrefetch, SubsecondTime t_issue, bool have_write_lock, IntPtr ca_pc)
{
   ...
   {
	std::cout<<"debug1 ca_pc:"<< ca_pc << std::endl;
      if(pc_accesses.find(ca_pc) == pc_accesses.end())
	{
	std::cout<<"debug1"<<std::endl;
		pc_hits.insert(std::make_pair(ca_pc, 0));
		pc_accesses.insert(std::make_pair(ca_pc, 0));
      }
...
/*void
CacheCntlr::updateCounters(Core::mem_op_t mem_op_type, IntPtr address, bool cache_hit, CacheState::cstate_t state, Prefetch::prefetch_type_t isPrefetch, IntPtr PC)
{
   /* If another miss to this cache line is still in progress:
      operationPermissibleinCache() will think it's a hit (so cache_hit == true) since the processing
      of the previous miss was done instantaneously. But mshr[address] contains its completion time */
   ...
   {
      if(pc_accesses.find(PC) == pc_accesses.end())
	{
		pc_hits[PC] = 0;
		pc_accesses[PC] = 0;
      }
...
CacheCntlr::inform_tlb_eviction(IntPtr addr, UInt32 way)
{
    //return;
    if (m_master->m_prefetcher) {
        std::vector<IntPtr> prefetchList = m_master->m_prefetcher->inform_tlb_eviction(addr, way);
