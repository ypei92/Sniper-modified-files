...
namespace ParametricDramDirectoryMSI
{
   ...
   class CacheCntlr : ::CacheCntlr
   {
	  ...
	  struct {
         ...
         void updateCounters(Core::mem_op_t mem_op_type, IntPtr address, bool cache_hit, CacheState::cstate_t state, Prefetch::prefetch_type_t isPrefetch, IntPtr ca_pc);
         void cleanupMshr();
         ...
         void trainPrefetcher(IntPtr pc, IntPtr address, bool cache_hit, bool prefetch_hit, SubsecondTime t_issue);
         void Prefetch(SubsecondTime t_start);
         void doPrefetch(IntPtr prefetch_address, IntPtr pc, SubsecondTime t_start);
